 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export ym_data=' wx-openid或者wx-token或者token的值 '   ,多账号用 换行 或 @ 分割
 * 
 */
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.ym_data : $.getdata('ym_data')) || '';
 let msg, ck;
 let ck_status = true;
 let host = 'ztjf.mallzto.com';
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "ym_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/api/index/doSignIn.do`,
			 headers: {
				'host': host,
				"Content-Length": "34",
		 ...
		 if (result.code == 200) {
			 DoubleLog(`签到成功，签到获得${result.content.signPoints} 积分，当前总积分${result.content.points}🎉`);
			 await wait(3);
